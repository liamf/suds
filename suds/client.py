...
class Client(object):
    ...
    def enableZendDebug(self, debugHost, debugPort):
        """
        Enabling Zend Studio debugging requires passing some data to the server in the Cookie header
        Most of the parameters are fixed, apart from the IP address of the Zend debug host, and the port
        So we just stash there in our parameters, and later on they will be added to the headers
        See the headers() method of SoapClient
        """
        self.set_options(zenddebug=1,zenddebughost=debugHost,zenddebugport=debugPort)
        
    def disableZendDebug(self):
        self.set_options(zenddebug=0)
...
class SoapClient:
    ...
    def headers(self):
        ...
        This is where we add in the Zend Cookie headers (which will trigger a debug session) if required
        ...
        p = Unskin(self.options)
        if p.get('zenddebug',0) == 1:
            # We want to enable Zend debugging via Cookie
            # To do that we need to send the following Cookie header:
            # debug_stop=1; debug_host=<IP>; debug_port=<port>; start_debug=1; send_debug_header=1;
            # no_remote=1; send_sess_end=1; debug_jit=1; ZDEDebuggerPresent=php,phtml,php3; debug_session_id=1234567;
            # In the above list, replace <IP> with the IP address of the machine running the Zend Studio for Eclipse, 
            # and <port> with the port used for communication (typically either 10000 or 10137). 
            # The debug_session_id is not too important, the "real" toolbar would send a different session id for each session 
            # but it does not seem to matter if you use the same one every time.
            stock["Cookie"] = "debug_stop=1;debug_host="+p.get('zenddebughost','localhost')+";debug_port="+str(p.get('zenddebugport',10000))+";start_debug=1;send_debug_header=1;no_remote=1;send_sess_end=1;debug_jit=1;ZDEDebuggerPresent=php,phtml,php3;debug_session_id=1234567;"
    ...
    # Return the SOAP URL, adding in Zend debug startup if required
    def location(self):
        p = Unskin(self.options)
